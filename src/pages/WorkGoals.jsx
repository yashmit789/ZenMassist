...
const WorkGoals = () => {
    ...
    const handleAddGoal = async (e) => {
        ...
        if (error) {
            ...
            setGoals([...goals, data[0]].sort((a, b) => new Date(a.target_date) - new Date(b.target_date)));
            setTitle(''); setDescription(''); setTargetDate('');
        }
    };

    const toggleComplete = async (id, currentStatus) => {
        const { error } = await supabase.from('goals').update({ completed: !currentStatus }).eq('id', id);
        if (!error) setGoals(goals.map(g => g.id === id ? { ...g, completed: !currentStatus } : g));
    };
    ...
    useEffect(() => {
        ...
        if (activeTimer) {
            interval = setInterval(() => {
                setGoals(prev => prev.map(g => g.id === activeTimer ? { ...g, duration_seconds: (g.duration_seconds || 0) + 1 } : g));
            }, 1000);
        }
    ...
    // Just replace the RETURN statement of your WorkGoals.jsx with this:
